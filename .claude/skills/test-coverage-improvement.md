...
```
Jest 2726 (单元, 55 文件: 36 WMS + 18 WF + 1 scripts)
  ├── WMS 前端: jsdom + Istanbul 手动插桩 (vm.Script 沙盒穿透)
  ├── n8n 纯函数: 标准 Jest (无 DOM 依赖)
  └── 工具脚本: add-ids.js (纯函数提取后可测)
Playwright 153 (E2E, 14 文件: 12 页面全覆盖 + 边界用例 + 移动端视口)
  └── Network Interception + python3 http.server
BATS ~170 (基建 Shell, 18 文件)
pytest 248 (基建 Python, 15 文件)
SQL ~145 (约束/触发器/视图, 16 文件: RAISE NOTICE 'PASS' 断言)
总计: ~3442
```

> **重要**: BATS 测试 `@test` 名称、注释、错误消息**必须用英文**。源脚本中文输出可在 assert 中匹配。
...
1. `CLAUDE.md` — 目录树注释 + 测试框架表 (文件数)
2. `docs/ADR/006-test-coverage-analysis.md` — 上下文行、统计表、总用例
3. `.claude/skills/test-coverage-improvement.md` — 金字塔数字
4. `docs/test-coverage-analysis-*.md` — 当次报告
5. 相关覆盖率地图 (Shell→BATS / PG init→SQL 映射)
