...
> **创建**: 2026-02-23
> **版本**: v3.0 (2026-03-05 重构: 分离历史归档 vs 持续适用)
...
| 问题 | 根因 | 解决 |
|------|------|------|
| `null value in column "id"` | 19/20 JSON 文件根节点是 `[{...}]` 数组，缺少 id 字段 | 导入前用 Python 提取 `data[0]` 并注入 hash id |
...
for fpath in glob("apps/wf/wf*.json"):
    ...
    if not wf.get('id'):
        wf['id'] = hashlib.md5(fpath.encode()).hexdigest()[:10]
...
EXECUTIONS_MODE: regular                       # v0.1 单节点
```
