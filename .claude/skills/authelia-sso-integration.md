...
> **版本**: v3.0 (2026-03-17)
> **适用**: Authelia v4.39.16 + nginx auth_request + Docker Compose
> **范围**: 从零搭建到多域名 SSO 全覆盖 + 应用角色映射的完整经验
...
## 5. Superset 6.0 SSO 集成 — FLASK_APP_MUTATOR + 角色同步
...
### 解决方案: before_request hook + 角色同步
```python
if os.environ.get('DP_SSO_ENABLED', 'false').lower() == 'true':
    from flask_appbuilder.security.manager import AUTH_REMOTE_USER
    AUTH_TYPE = AUTH_REMOTE_USER
    AUTH_USER_REGISTRATION = True
    AUTH_USER_REGISTRATION_ROLE = 'Gamma'  # 最小权限, 实际由角色映射覆盖
    AUTH_ROLES_MAPPING = {
        'admins': ['Admin'],      # Authelia admins → Superset Admin
        'bi-users': ['Alpha'],    # Authelia bi-users → Superset Alpha
    }
    ...
    def FLASK_APP_MUTATOR(app):
        ...
        def sso_remote_user_auth():
            ...
            if user:
                login_user(user)
                _sync_roles_from_groups(app, user, request)  # 角色同步
                # ...

    def _sync_roles_from_groups(app, user, request):
        """根据 Authelia X-Forwarded-Groups header 同步 Superset 角色"""
        groups_header = request.environ.get('HTTP_X_FORWARDED_GROUPS', '')
        groups = [g.strip() for g in groups_header.split(',') if g.strip()]
        target_role_names = set()
        for group in groups:
            if group in roles_mapping:
                target_role_names.update(roles_mapping[group])
        if not target_role_names:
            return  # 无匹配组, 保持 Gamma
        # 查询角色对象, 仅在角色不一致时更新
        target_roles = db.session.query(sm.role_model).filter(
            sm.role_model.name.in_(target_role_names)
        ).all()
        if set(r.name for r in user.roles) != set(r.name for r in target_roles):
...
| 层级 | 控制点 | 作用 |
|------|--------|------|
| 第一层 | Authelia `configuration.yml` access_control | 谁能进哪个模块 (网关门禁) |
| 第二层 | Superset `AUTH_ROLES_MAPPING` | 进入 BI 后能干什么 (应用内角色) |

### Authelia 组 → Superset 角色映射
| Authelia 组 | Superset 角色 | 权限范围 |
|-------------|--------------|---------|
| `admins` | Admin | 全权限 (含数据库连接管理 + 用户管理) |
| `bi-users` | Alpha | SQL Lab + 创建图表/仪表板 (不能编辑数据库连接) |
| 无匹配组 | Gamma (注册默认) | 仅查看被授权的仪表板 |

### Alpha vs Admin 数据库权限对比
| 权限 | Alpha | Admin | 说明 |
|------|:-----:|:-----:|------|
| `can_read on Database` | 有 | 有 | 查看数据库连接列表 |
| `can_write on Database` | **无** | 有 | 创建/编辑/删除数据库连接 |
| `can_upload on Database` | 有 | 有 | CSV 上传到数据库 |
| `all_database_access` | 有 | 有 | SQL Lab 查询所有数据库 |
| `can_write on Dataset` | 有 | 有 | 编辑数据集 (易与 Database 混淆) |

### 关键教训
| 教训 | 说明 |
|------|------|
| `HTTP_REMOTE_USER` 不是 `REMOTE_USER` | nginx `Remote-User` header → WSGI `HTTP_REMOTE_USER` (带 HTTP_ 前缀) |
| Groups header = `HTTP_X_FORWARDED_GROUPS` | nginx `X-Forwarded-Groups` → WSGI `HTTP_X_FORWARDED_GROUPS` |
| `AUTH_USER_REGISTRATION_ROLE` 改为 `Gamma` | 最小权限原则, 实际角色由 `AUTH_ROLES_MAPPING` 动态同步 |
| `Public` 角色绝不可用 | Public = 所有 API 返回 403 = 页面白屏/无限加载 |
...
| 角色同步仅在首次认证触发 | `g.user.is_authenticated` 检查确保已登录用户不重复同步 |
| Alpha 不能编辑数据库连接 | 常被混淆: `can_write on Dataset` ≠ `can_write on Database` |
...
| BI 全白屏/403 | SSO 用户角色为 Public 或 Gamma 无授权仪表板 | 检查 users.yml 中是否有 `bi-users` 组; 无组 = Gamma (仅查看被授权的) |
| IT 能编辑数据库连接? | Alpha 无 `can_write on Database` | 可能混淆 Dataset 编辑与 Database 编辑, 或 UI 显示按钮但保存 403 |
| 角色未从 SSO 同步 | `X-Forwarded-Groups` 未传递 | 检查 sso-headers.inc 是否包含 `proxy_set_header X-Forwarded-Groups` |
