 ...
 * @param {string} companyCode - 公司代码 (必填，v0.5 新增)
 * @param {number} [batchSize=200] - 每批大小
 * @returns {Array} [{ sql, batch_num, count }]
 */
function buildItemsUpsertBatches(items, companyCode, batchSize) {
    if (!companyCode || String(companyCode).trim() === '') {
        throw new Error('companyCode 不能为空');
    }
    batchSize = batchSize || 200;
    var cc = escapeValue(companyCode, 20);
    var batches = [];
    for (var i = 0; i < items.length; i += batchSize) {
        var batch = items.slice(i, i + batchSize);
        var values = batch.map(function(j) {
            return "('" + cc + "','" + escapeValue(j.ItemCode) + "','" + escapeValue(j.ItemName) +
                "','" + escapeValue(j.InvntryUom) + "','" + (escapeValue(j.ManBtchNum) || 'N') + "',NOW())";
        }).join(',\n');
        var sql = 'INSERT INTO wms.wms_items_cache (company_code, item_code, item_name, uom, man_batch_num, synced_at) VALUES ' +
            values + ' ON CONFLICT (company_code, item_code) DO UPDATE SET item_name=EXCLUDED.item_name, uom=EXCLUDED.uom, man_batch_num=EXCLUDED.man_batch_num, synced_at=NOW()';
 ...
 */
function buildLocationsUpsert(items, companyCode) {
    if (!companyCode || String(companyCode).trim() === '') {
        throw new Error('companyCode 不能为空');
    }
    ...
    var values = items.map(function(j) {
        return "('" + cc + "','" + escapeValue(j.WhsCode) + "','" + escapeValue(j.WhsName) + "',NOW())";
    }).join(',\n');
    var sql = 'INSERT INTO wms.wms_locations_cache (company_code, whs_code, whs_name, synced_at) VALUES ' +
        values + ' ON CONFLICT (company_code, whs_code) DO UPDATE SET whs_name=EXCLUDED.whs_name, synced_at=NOW()';
 ...
 * @returns {Array} [{ sql, batch_num, count }]
 */
function buildBinsUpsertBatches(items, companyCode, batchSize) {
    if (!companyCode || String(companyCode).trim() === '') {
        throw new Error('companyCode 不能为空');
    }
    batchSize = batchSize || 200;
    ...
    for (var i = 0; i < items.length; i += batchSize) {
        var batch = items.slice(i, i + batchSize);
        var values = batch.map(function(j) {
            return "('" + cc + "','" + escapeValue(j.bin_code) + "','" + escapeValue(j.whs_code) +
                "','" + escapeValue(j.whs_name) + "','" + escapeValue(j.bin_name) +
                "'," + safeNum(j.max_level) + ",NOW())";
        }).join(',\n');
        var sql = 'INSERT INTO wms.wms_bins_cache (company_code, bin_code, whs_code, whs_name, bin_name, max_level, synced_at) VALUES ' +
            values + ' ON CONFLICT (company_code, bin_code) DO UPDATE SET whs_code=EXCLUDED.whs_code, whs_name=EXCLUDED.whs_name, bin_name=EXCLUDED.bin_name, max_level=EXCLUDED.max_level, synced_at=NOW()';
        batches.push({ sql: sql, batch_num: Math.floor(i / batchSize) + 1, count: batch.length });
    }
    if (batches.length === 0) return [{ sql: 'SELECT 1', batch_num: 0, count: 0 }];
    return batches;
