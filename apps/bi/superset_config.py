# =============================================================================
# DP v0.4 — BI 引擎配置 (适配 Superset 6.1.0 + FAB 5.0.2)
# PostgreSQL 元数据后端 (Schema: bi) + Redis 缓存 (dp-cache-bi)
# =============================================================================

import os

# ---------------------------------------------------------------------------
# 密钥 (必须在 .env 中设置!)
# ---------------------------------------------------------------------------
SECRET_KEY = os.environ.get('SUPERSET_SECRET_KEY', '')

# ---------------------------------------------------------------------------
# 元数据数据库 (PostgreSQL, Schema: bi)
# ---------------------------------------------------------------------------
_db_host = os.environ.get('DP_DB_HOST', 'dp-db')
_db_port = os.environ.get('DP_DB_PORT', '5432')
_db_name = os.environ.get('DP_DB_NAME', 'dp')
_db_user = os.environ.get('DP_DB_USER', 'dp_app')
_db_pass = os.environ.get('DP_DB_PASSWORD', '')

SQLALCHEMY_DATABASE_URI = (
    f'postgresql+psycopg2://{_db_user}:{_db_pass}@{_db_host}:{_db_port}/{_db_name}'
    f'?options=-c%20search_path%3Dbi%2Cwms&sslmode=require'
)

# ---------------------------------------------------------------------------
# Redis 缓存 (dp-cache-bi)
# ---------------------------------------------------------------------------
_redis_host = os.environ.get('DP_REDIS_BI_HOST', 'dp-cache-bi')
_redis_port = os.environ.get('DP_REDIS_BI_PORT', '6379')
_redis_pass = os.environ.get('DP_REDIS_BI_PASSWORD', '')

CACHE_CONFIG = {
    'CACHE_TYPE': 'RedisCache',
    'CACHE_DEFAULT_TIMEOUT': 300,
    'CACHE_KEY_PREFIX': 'superset_',
    'CACHE_REDIS_URL': f'redis://:{_redis_pass}@{_redis_host}:{_redis_port}/0',
}

DATA_CACHE_CONFIG = {
    'CACHE_TYPE': 'RedisCache',
    'CACHE_DEFAULT_TIMEOUT': 600,
    'CACHE_KEY_PREFIX': 'superset_data_',
    'CACHE_REDIS_URL': f'redis://:{_redis_pass}@{_redis_host}:{_redis_port}/0',
}

FILTER_STATE_CACHE_CONFIG = {
    'CACHE_TYPE': 'RedisCache',
    'CACHE_DEFAULT_TIMEOUT': 600,
    'CACHE_KEY_PREFIX': 'superset_filter_',
    'CACHE_REDIS_URL': f'redis://:{_redis_pass}@{_redis_host}:{_redis_port}/0',
}

# Celery (异步查询)
class CeleryConfig:
    broker_url = f'redis://:{_redis_pass}@{_redis_host}:{_redis_port}/1'
    result_backend = f'redis://:{_redis_pass}@{_redis_host}:{_redis_port}/1'
    task_annotations = {
        'sql_lab.get_sql_results': {
            'rate_limit': '100/m',
        },
    }

CELERY_CONFIG = CeleryConfig

# ---------------------------------------------------------------------------
# 路径前缀 (通过 nginx /bi/ 访问)
# ---------------------------------------------------------------------------
ENABLE_PROXY_FIX = True

# ---------------------------------------------------------------------------
# 功能标志
# ---------------------------------------------------------------------------
FEATURE_FLAGS = {
    'ENABLE_TEMPLATE_PROCESSING': True,
    # DASHBOARD_NATIVE_FILTERS / DASHBOARD_CROSS_FILTERS: Superset 6.1 已永久启用，从配置中移除
    'EMBEDDED_SUPERSET': True,
}

# ---------------------------------------------------------------------------
# 时区
# ---------------------------------------------------------------------------
BABEL_DEFAULT_LOCALE = 'zh'
BABEL_DEFAULT_FOLDER = 'superset/translations'
LANGUAGES = {
    'en': {'flag': 'us', 'name': 'English'},
    'zh': {'flag': 'cn', 'name': 'Chinese'},
    'th': {'flag': 'th', 'name': 'Thai'},
}

# ---------------------------------------------------------------------------
# 品牌 / Logo 跳转路径
# ---------------------------------------------------------------------------
# 点击 Logo 跳转到 BI 欢迎页 (而非根路径 / 导航页)
LOGO_TARGET_PATH = '/superset/welcome/'

# 必须同步覆盖 THEME_DEFAULT，否则 React 前端仍使用默认 brandLogoHref="/"
THEME_DEFAULT = {
    'token': {
        'brandLogoHref': '/superset/welcome/',
    },
}

# ---------------------------------------------------------------------------
# 登出重定向 (防止回到网关 landing 页)
# ---------------------------------------------------------------------------
# 问题链路: 退出 → /login/ (无 next) → 登录成功 → FAB 默认跳 / → nginx = / → landing
# 修复: 携带 next 参数，登录成功后直接回 BI 欢迎页
LOGOUT_REDIRECT_URL = '/login/?next=/superset/welcome/'

# ---------------------------------------------------------------------------
# 安全
# ---------------------------------------------------------------------------
WTF_CSRF_ENABLED = True
SESSION_COOKIE_HTTPONLY = True
SESSION_COOKIE_SECURE = False  # 生产环境改为 True
TALISMAN_ENABLED = False       # 生产环境改为 True
RECAPTCHA_PUBLIC_KEY = ''      # Superset 6.x 需要此键 (不启用 reCAPTCHA)

# ---------------------------------------------------------------------------
# SSO 统一认证 (Authelia Remote-User header)
# ---------------------------------------------------------------------------
# nginx auth_request → Authelia 验证 → X-Forwarded-User header 注入
# FAB AUTH_REMOTE_USER_HEADER: 直接指定 WSGI environ 变量名 (HTTP_X_FORWARDED_USER)
# 无需 WSGI middleware — FAB 原生支持此配置
# 注意: ENABLE_PROXY_FIX 必须全局生效 (已在上面启用), 否则 Superset 无法识别代理 header
if os.environ.get('DP_SSO_ENABLED', 'false').lower() == 'true':
    from flask_appbuilder.security.manager import AUTH_REMOTE_USER
    AUTH_TYPE = AUTH_REMOTE_USER
    # 自动创建 Superset 内部用户 (非自注册, 仅从 SSO header 同步)
    AUTH_USER_REGISTRATION = True
    # SSO 新用户默认 Gamma (最小权限, 仅查看被授权的仪表板)
    # 实际角色由 AUTH_ROLES_MAPPING 根据 Authelia groups 动态同步
    AUTH_USER_REGISTRATION_ROLE = 'Gamma'
    # Authelia groups → Superset 角色映射
    # admins (SSO 管理员) → Admin (Superset 全权限)
    # bi-users (BI 用户) → Alpha (SQL Lab + 创建图表/仪表板, 但不能编辑数据库连接)
    # 无匹配组 → 保持 Gamma (仅查看)
    AUTH_ROLES_MAPPING = {
        'admins': ['Admin'],
        'bi-users': ['Alpha'],
    }
    AUTH_ROLES_SYNC_AT_LOGIN = True

    # SSO 模式下登出重定向到 Authelia
    LOGOUT_REDIRECT_URL = '/auth/logout'

    # -----------------------------------------------------------------------
    # Superset 6.1 SSO 修复: before_request 注入 Remote-User 认证
    # -----------------------------------------------------------------------
    # 问题: Superset 6.x SupersetAuthView.login() 完全覆盖 FAB AuthRemoteUserView
    #        从不读取 REMOTE_USER environ — 只渲染登录页 React SPA
    # 修复: FLASK_APP_MUTATOR + before_request hook 绕过 Superset 的 login view
    #        在每个请求前检查 HTTP_REMOTE_USER, 自动认证+创建用户
    # 附加: /login/ 路径直接重定向到 welcome (防止 Superset 内部 302 → /login/ 循环)
    #
    # FAB 5.0.2 迁移: auth_user_remote_user() → find_user + add_user + update_user_auth_stat
    # 原因: Flask 2→3 后 auth_user_remote_user 内部 session 处理不稳定
    def FLASK_APP_MUTATOR(app):
        @app.before_request
        def sso_remote_user_auth():
            from flask import request, g, redirect
            from flask_login import login_user
            username = request.environ.get('HTTP_REMOTE_USER')
            if not username:
                return  # 非 SSO 请求 (bypass 路径或内部调用)
            if g.user is not None and g.user.is_authenticated:
                # 已认证 — 但如果访问 /login/ 需要重定向走 (防循环)
                if request.path == '/login/' or request.path == '/login':
                    next_url = request.args.get('next', '/superset/welcome/')
                    return redirect(next_url)
                return
            # FAB 5.0.2 三步认证: find_user → add_user → update_user_auth_stat
            # 替换已废弃的 auth_user_remote_user (Flask 3 + SQLAlchemy 2.0 session 问题)
            sm = app.appbuilder.sm
            user = sm.find_user(username=username)
            if not user:
                default_role = sm.find_role(
                    app.config.get('AUTH_USER_REGISTRATION_ROLE', 'Gamma')
                )
                user = sm.add_user(
                    username=username,
                    first_name=username,
                    last_name='',
                    email=f'{username}@sso.local',
                    role=default_role,
                )
                if not user:
                    app.logger.error(f'SSO 用户创建失败: {username}')
                    return
                sm.session.commit()
                app.logger.info(f'SSO 自动创建用户: {username}')
            # 关键三步之三: 更新认证统计 (last_login / login_count)
            # auth_user_remote_user 内部会调用此方法，手动迁移后必须显式调用
            sm.update_user_auth_stat(user)
            login_user(user)
            # --- 角色同步: Authelia groups → Superset 角色 ---
            # nginx 转发 X-Forwarded-Groups header (Authelia 验证后注入)
            _sync_roles_from_groups(app, user, request)
            # 登录成功后如果在 /login/ 页, 直接跳转 (防 Superset 渲染 React 登录页)
            if request.path == '/login/' or request.path == '/login':
                next_url = request.args.get('next', '/superset/welcome/')
                return redirect(next_url)

    def _sync_roles_from_groups(app, user, request):
        """根据 Authelia X-Forwarded-Groups header 同步 Superset 角色"""
        if not app.config.get('AUTH_ROLES_SYNC_AT_LOGIN'):
            return
        roles_mapping = app.config.get('AUTH_ROLES_MAPPING', {})
        if not roles_mapping:
            return
        groups_header = request.environ.get('HTTP_X_FORWARDED_GROUPS', '')
        if not groups_header:
            return
        groups = [g.strip() for g in groups_header.split(',') if g.strip()]
        # 收集目标角色名
        target_role_names = set()
        for group in groups:
            if group in roles_mapping:
                target_role_names.update(roles_mapping[group])
        if not target_role_names:
            return  # 无匹配组, 保持当前角色 (注册默认 Gamma)
        # 查询角色对象 — 使用 sm.session 替代 db.session (FAB 5.0.2 + SQLAlchemy 2.0)
        sm = app.appbuilder.sm
        try:
            target_roles = sm.session.query(sm.role_model).filter(
                sm.role_model.name.in_(target_role_names)
            ).all()
            if not target_roles:
                return
            # 仅在角色不一致时更新 (减少不必要的 DB 写入)
            current = set(r.name for r in user.roles)
            desired = set(r.name for r in target_roles)
            if current != desired:
                user.roles = target_roles
                sm.session.commit()
        except Exception as e:
            sm.session.rollback()
            app.logger.error(f'角色同步失败: {e}')
