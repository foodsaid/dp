...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && typeof CONFIG !== 'undefined') {
    ...
    function renderOrder(data) {
        ...
        if (headerClosed || !hasOpenLines) {
            ...
            banner.innerHTML = '<span style="font-size:1.3rem;color:#16a34a;font-weight:bold;">&#10003; 该订单已全部完成发料</span>';
    ...
    function selectLine(lineNum) {
        ...
        if (isPiHeaderClosed(order.status)) { showMessage('订单已关闭，无法作业', 'error'); playErrorSound(); focusScanInput(); return; }
    ...
    var oneClickIssueAll = async function() {
        ...
        var order = currentOrder.sap_order || {};
        if (isPiHeaderClosed(order.status)) { showMessage('订单已关闭，无法发料', 'error'); return; }
        var wms = currentOrder.wms_history || {};
