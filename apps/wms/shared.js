...
/**
 * 带指数退避重试的订单加载 — 当 OMS 同步未完成时自动重试
 * @param {string} endpoint - API 路径 (如 /wms/so/12345)
 * @param {number} maxRetries - 最大重试次数 (默认 2)
 * @returns {Object} API 响应
 */
async function loadOrderWithRetry(endpoint, maxRetries) {
    if (maxRetries === undefined) maxRetries = 2;
    for (var i = 0; i <= maxRetries; i++) {
        var data = await apiGet(endpoint);
        if (data && data.success) return data;
        if (data && data.error_type === 'sync_pending' && i < maxRetries) {
            var delay = 300 * Math.pow(2, i) + Math.random() * 200;
            if (typeof showMessage === 'function') {
                showMessage(t('msg.sync_pending', [Math.ceil(delay / 1000)]), 'info');
            }
    ...
    return { success: false, error_type: 'sync_timeout', message: t('msg.sync_timeout') };
}
...
function setupBarcodeInput(elementId, callback) {
    ...
    input.addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Enter') {
            ...
            if (_isLoadingDoc) {
                input.value = '';
                showMessage(t('msg.doc_loading_wait', '单据加载中，请稍候...'), 'warning');
                return;
            }
    ...
    input.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') {
            ...
            if (!isNaN(val) && max > 0 && val > max * 10) {
                ...
                e.stopPropagation();
                playErrorSound();
                showMessage(t('msg.qty_abnormal_blocked', '数量异常 ({0})，疑似扫码枪误触，已拦截').replace('{0}', formatNumber(val)), 'error');
                input.value = max;
                input.select();
                return false;
            }
...
function routeBarcode(barcode) {
    ...
    for (var i = 0; i < oldPrefixes.length; i++) {
        if (upper.startsWith(oldPrefixes[i])) {
            ...
            playWarningSound();
            showMessage(t('msg.old_barcode_format', '旧格式条码! 请使用新格式: {0} (无连字符)').replace('{0}', typeName + num), 'warning');
            return false;
        }
    }
    ...
    if (/^\d+$/.test(barcode)) {
        playWarningSound();
        showMessage(t('msg.numeric_barcode_hint', '纯数字条码，请使用前缀格式 (如 WO{0}, PO{0})').replace(/\{0\}/g, barcode), 'warning');
        return false;
    }
    ...
    // 无法识别
    playErrorSound();
    showMessage(t('msg.barcode_unrecognized', '无法识别条码: {0}').replace('{0}', escapeHtml(barcode)), 'warning');
    return false;
}
...
function handleSubpageBarcode(barcode, currentPrefix, loadFn, filterFn) {
    ...
    for (var p in sapPrefixes) {
        if (p !== currentUpper && upper.startsWith(p) && /^\d+$/.test(barcode.substring(p.length))) {
            if (_mismatchBarcode !== barcode) {
                ...
                playWarningSound();
                showMessage(t('msg.doc_mismatch_enter_again', '单据不匹配! 当前{0}, 条码是{1}。再次回车跳转').replace('{0}', currentPrefix).replace('{1}', p), 'warning');
                return;
            }
            _mismatchBarcode = null;
            // DD 保留完整单号, 其他剥离前缀
    ...
    for (var oi = 0; oi < oldPrefixes.length; oi++) {
        if (upper.startsWith(oldPrefixes[oi])) {
            ...
            _mismatchBarcode = null;
            showMessage(t('msg.old_barcode_format', '旧格式条码! 请使用新格式: {0} (无连字符)').replace('{0}', typeName2 + num2), 'warning');
            playWarningSound();
            return;
        }
    }
    ...
    if (barcode.indexOf('-') >= 0 || barcode.length > 8) {
        ...
        if (typeof filterFn === 'function') {
            ...
            playWarningSound();
            showMessage(t('msg.item_not_supported_here', '物料代码: {0}，当前页面不支持物料过滤').replace('{0}', escapeHtml(barcode)), 'warning');
        }
        return;
    }
...
function filterLineByItemCode(scannedCode, lines, selectLineFn, checkCompleteFn, options) {
    ...
    if (options && options.docStatus === 'completed') {
        playWarningSound();
        showMessage(t('msg.doc_completed_no_scan', '当前单据已完成，无法继续扫码作业'), 'warning');
        focusScanInput();
        return;
    }

    if (!lines || lines.length === 0) {
        showMessage(t('msg.load_doc_first', '请先加载单据'), 'warning');
        return;
    }
    ...
    // 无匹配
    if (matched.length === 0) {
        showMessage(t('msg.item_not_in_doc', '当前单据不包含物料: {0}').replace('{0}', escapeHtml(scannedCode)), 'error');
        playErrorSound();
        focusScanInput();
        return;
    }
    ...
    if (matched.length === 1) {
        if (typeof checkCompleteFn === 'function') {
            ...
            if (checkResult && checkResult.isComplete) {
                playErrorSound();
                if (!confirm(t('confirm.line_done_continue', '该行已完成 (剩余: {0})，是否继续录入？').replace('{0}', formatNumber(checkResult.remaining)))) {
                    focusScanInput();
                    return;
                }
            }
    ...
    if (actionable.length === 0) {
        playWarningSound();
        showMessage(t('msg.item_all_done', '该物料已全部作业完毕'), 'warning');
        focusScanInput();
        return;
    }
...
function validateRequired(fields) {
    for (var i = 0; i < fields.length; i++) {
        ...
        if (!val) {
            showMessage(t('msg.fill_field', '请填写: {0}').replace('{0}', fields[i].name), 'error');
            el.focus();
            return false;
        }
    }
...
function getStatusLabel(status) {
    // 优先使用 i18n 翻译 (lang.js 的 wms.status.xxx)
    if (typeof t === 'function') {
        var i18nKey = 'wms.status.' + status;
...
function renderHeaderStatus(sapStatus, wmsStatus, sapElId, wmsElId) {
    var sapLabel = (sapStatus === 'O') ? t('sap.status.O', '未清') :
                   (sapStatus === 'C') ? t('sap.status.C', '已关闭') :
                   (sapStatus === 'R') ? t('sap.status.R', '已下达') :
                   (sapStatus === 'P') ? t('sap.status.P', '已计划') :
                   (sapStatus === 'L') ? t('sap.status.L', '已关闭') :
                   sapStatus || '-';
    ...
    input.addEventListener('input', function () {
        ...
        if (!isNaN(val) && val > max && max > 0) {
            ...
            if (now - warningThrottle > 2000) {
                warningThrottle = now;
                playWarningSound();
                showMessage(t('msg.qty_over_remaining', '数量 {0} 超过剩余 {1}!').replace('{0}', formatNumber(val)).replace('{1}', formatNumber(max)), 'warning');
            }
        } else {
...
function validateOverQty(qty, remaining, remark, remarkInputId, confirmMsg) {
    ...
    if (qty > remaining && remaining > 0) {
        if (!remark) {
            playWarningSound();
            showMessage(t('msg.qty_over_must_remark', '数量超过剩余! 如需超收/超发, 请在备注中填写原因'), 'error');
            var remarkEl = document.getElementById(remarkInputId);
            if (remarkEl) remarkEl.focus();
            return false;
        }
        if (!confirm(t('confirm.qty_over_continue', '⚠ 数量 {0} 超过剩余 {1}!\n备注: {2}\n确认继续?').replace('{0}', formatNumber(qty)).replace('{1}', formatNumber(remaining)).replace('{2}', remark))) {
            return false;
        }
        return true;
    }
...
async function batchSubmitAll(openLines, buildPayloadFn, actionLabel) {
    if (openLines.length === 0) {
        showMessage(t('msg.no_open_lines', '没有待处理的行项目'), 'warning');
        return false;
    }
    // V17.1: 防止一键按钮重复点击
    ...
    if (errors.length > 0) {
        showMessage(t('result.one_click_partial', '完成 {0}/{1}, 失败: {2}').replace('{0}', successCount).replace('{1}', openLines.length).replace('{2}', errors.join('; ')), 'warning');
        playErrorSound();
    } else {
...
async function loadMasterDataCache(forceRefresh) {
    ...
    try {
        ...
        showMessage(t('msg.master_data_load_fail', '⚠ 主数据加载失败，仓库/库位校验不可用，请联系管理员'), 'error');
    }
}
    ...
    try {
        ...
        tagBox.addEventListener('dblclick', function (e) {
            ...
            if (chip) {
                ...
                renderTags();
                if (typeof showMessage === 'function') showMessage(t('msg.bin_shortcut_removed', '已移除快捷库位 [{0}]').replace('{0}', binCode), 'info');
            }
        });
 ...
 * @param {*} ref - 引用 (检查 typeof !== 'undefined')
 * @returns {boolean} 是否可用
 */
function checkDep(name, ref) {
    if (typeof ref === 'undefined' || ref === null) {
        console.error('[WMS] ' + name + ' 未加载');
        return false;
    }
    return true;
}
