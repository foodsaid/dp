...
function buildPoLineRowHtml(line, wms, opts, h) {
    ...
    var rowHtml = '<tr class="' + (lineDone ? 'line-done' : '') + '" data-line="' + line.lineNum + '">' +
        ...
        '<td class="action-col no-print">' + (lineDone ? '<span class="badge badge-completed">已完成</span>' : '<button class="btn btn-primary" style="padding:4px 12px;font-size:0.8rem;" onclick="selectLine(' + line.lineNum + ')">收货</button>') + '</td>' +
        '</tr>';
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && typeof CONFIG !== 'undefined') {
    ...
    function renderOrder(data) {
        ...
        if (headerClosed || !hasOpenLines) {
            ...
            banner.innerHTML = '<span style="font-size:1.3rem;color:#16a34a;font-weight:bold;">&#10003; 该订单已全部完成收货</span>';
    ...
    function selectLine(lineNum) {
        ...
        if (order.docStatus !== 'O') { showMessage('订单已关闭，无法作业', 'error'); playErrorSound(); focusScanInput(); return; }
    ...
    var oneClickReceiveAll = async function() {
        ...
        var order = currentOrder.sap_order || {};
        if (order.docStatus !== 'O') { showMessage('订单已关闭，无法收货', 'error'); return; }
        var wms = currentOrder.wms_history || {};
