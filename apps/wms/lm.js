...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && typeof CONFIG !== 'undefined') {
    ...
    var handleCreate = async function(event) {
        ...
        if (!whs || !user) { showMessage(t('msg.fill_whs_and_user', '请填写仓库和操作人'), 'error'); return; }
        ...
        try {
            ...
            showMessage(t('result.lm_doc_created', '移库单创建成功: {0}').replace('{0}', result.doc_number), 'success');
    ...
    var lookupItem = async function(code) {
        ...
        if (dashParts.length > 2 && code.length > 15) {
            showMessage(t('msg.item_code_abnormal', '物料号异常 (可能两个条码被合并): {0}\n请重新扫描单个物料').replace('{0}', escapeHtml(code)), 'error');
            playErrorSound();
            return;
        }
        try {
            ...
            if (data && data.success && data.item) {
                ...
                var itemMsg = (data && data.error_type === 'item_not_cached')
                    ...
                    : t('msg.item_not_in_sap', '物料 {0} 未在SAP中找到，不允许操作').replace('{0}', escapeHtml(code));
    ...
    var handleMove = async function(event) {
        ...
        if (!itemCode || itemCode === '-') { showMessage(t('msg.scan_item_first', '请先扫描物料条码'), 'error'); return; }
        ...
        if (fromResult === false) {
            ...
            showMessage(t('msg.bin_not_in_master', '库位代码 [{0}] 不在主数据中，请重新输入').replace('{0}', fromBin), 'error');
            return;
        }
        if (fromResult === null) {
            showMessage(t('msg.bin_dict_not_loaded', '库位字典未加载，无法校验库位 [{0}]，请刷新页面').replace('{0}', fromBin), 'error');
            return;
        }
        ...
        if (toResult === false) {
            ...
            showMessage(t('msg.bin_not_in_master', '库位代码 [{0}] 不在主数据中，请重新输入').replace('{0}', toBin), 'error');
            return;
        }
        if (toResult === null) {
            showMessage(t('msg.bin_dict_not_loaded', '库位字典未加载，无法校验库位 [{0}]，请刷新页面').replace('{0}', toBin), 'error');
            return;
        }
        ...
        if (existInPending) {
            if (!confirm(t('confirm.lm_duplicate_pending', '待提交清单中已有 {0} {1} → {2} (数量: {3})\n是否继续追加一行?').replace('{0}', itemCode).replace('{1}', fromBin).replace('{2}', toBin).replace('{3}', formatNumber(existInPending.qty)))) return;
        }
        ...
        if (existingTx) {
            if (!confirm(t('confirm.lm_duplicate_tx', '已有提交记录: {0} {1} → {2} (数量: {3})\n是否继续追加?').replace('{0}', itemCode).replace('{1}', fromBin).replace('{2}', toBin).replace('{3}', formatNumber(existingTx.quantity)))) return;
        }
        ...
        renderPending();
        showMessage(t('result.lm_line_added', '已添加: {0} {1} → {2} × {3}').replace('{0}', itemCode).replace('{1}', fromBin).replace('{2}', toBin).replace('{3}', formatNumber(qty)), 'success');
        playSuccessSound();
