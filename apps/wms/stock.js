    ...
    if (whsEl) {
        ...
        whsEl.addEventListener('blur', function() {
            ...
            if (!result) {
                ...
                showMessage('仓库 ' + raw + ' 不在主数据缓存中', 'warning');
            } else {
    ...
    if (binEl) {
        ...
        binEl.addEventListener('blur', function() {
            ...
            if (!result) {
                ...
                showMessage('库位 ' + raw + ' 不在主数据缓存中', 'warning');
            } else {
...
function exportCSV() {
    if (currentStockData.length === 0) { showMessage('没有可导出的数据', 'warning'); return; }
    ...
    showMessage('已导出 ' + groups.length + ' 种物料, ' + currentStockData.length + ' 条明细', 'success');
}
