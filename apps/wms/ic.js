...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && typeof CONFIG !== 'undefined') {
        ...
        if (createWhsEl) {
            createWhsEl.addEventListener('blur', async function() {
                ...
                if (result === false) {
                    ...
                    createWhsEl.value = '';
                    showMessage('仓库代码 [' + raw + '] 不在主数据中，已清空，请重新输入', 'error');
                } else {
                    // 自动纠正为字典值 (PG 大小写敏感)
    ...
    var handleCreate = async function(event) {
        ...
        if (!whs || !user) { showMessage('请填写仓库和盘点人', 'error'); return; }
        ...
        try {
            ...
            showMessage('盘点单创建成功: ' + result.doc_number, 'success');
        ...
        if (cached) {
            ...
            try {
                ...
                if (data && data.success && data.item) {
                    ...
                    hide('itemInfo');
                    _currentStockData = [];
                    showMessage('物料 ' + escapeHtml(code) + ' 未在SAP中找到，不允许盘点', 'error');
                    playErrorSound();
                    return;
                }
            } catch (e) {
    ...
    var handleCount = async function(event) {
        ...
        if (isNaN(qty)) { showMessage('请输入有效数量', 'error'); return; }
        ...
        if (bin) {
            ...
            if (binResult === false) {
                ...
                showMessage('库位代码 [' + bin + '] 不在主数据中，请重新输入', 'error');
                return;
            }
            if (binResult === null) {
                showMessage('库位字典未加载，无法校验库位 [' + bin + ']，请刷新页面', 'error');
                return;
            }
            // 纠正为字典实际值
        ...
        renderPendingCounts();
        showMessage('已添加: ' + itemCode + ' x ' + formatNumber(qty) + (bin ? ' @ ' + bin : ''), 'success');
        playSuccessSound();
