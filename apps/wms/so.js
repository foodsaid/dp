...
function buildSoLineRowHtml(line, wms, opts, h) {
    ...
    if (opts.isDD) {
        ...
        var rowHtml = '<tr class="' + (lineDone ? 'line-done' : '') + '" data-line="' + line.lineNum + '">' +
            ...
            '<td class="action-col no-print">' + (lineDone ? '<span class="badge badge-completed">' + (typeof t === 'function' ? t('badge.completed', '已完成') : '已完成') + '</span>' : '<button class="btn btn-primary" style="padding:4px 12px;font-size:0.8rem;" onclick="selectLine(' + line.lineNum + ')">' + (typeof t === 'function' ? t('btn.pick', '拣货') : '拣货') + '</button>') + '</td></tr>';
...
if (typeof window !== 'undefined' && typeof document !== 'undefined' && typeof CONFIG !== 'undefined') {
    ...
    function renderOrder(data) {
        ...
        if (wmsStatus === 'split') {
            ...
            splitBanner.innerHTML = '<span style="font-size:1.3rem;color:#9d174d;font-weight:bold;">&#9888; ' + t('banner.split_to_dd', '该订单已拆分为DD，请使用DD单号进行拣货作业') + '</span>';
            ...
            banner.innerHTML = '<span style="font-size:1.3rem;color:#16a34a;font-weight:bold;">&#10003; ' + t('banner.all_picked', '该订单已全部完成拣货') + '</span>';
    ...
    function selectLine(lineNum) {
        if ((currentOrder.sap_order || {}).docStatus !== 'O') { showMessage(t('msg.order_closed', '订单已关闭，无法作业'), 'error'); playErrorSound(); focusScanInput(); return; }
        var line = ((currentOrder.sap_order || {}).lines || []).find(function (l) { return String(l.lineNum) === String(lineNum); });
    ...
    var oneClickPickAll = async function() {
        ...
        var order = currentOrder.sap_order || {};
        if (order.docStatus !== 'O') { showMessage(t('msg.order_closed_no_pick', '订单已关闭，无法拣货'), 'error'); return; }
        var wms = currentOrder.wms_history || {};
        ...
        var done = await batchSubmitAll(openLines, function (l) {
            return {
                ...
                action: 'scan', remarks: t('remark.one_click_pick', '一键拣货'),
                planned_qty: l.quantity, uom: l.uom
            };
        }, t('remark.one_click_pick', '一键拣货'));
