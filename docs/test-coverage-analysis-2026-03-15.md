...
| 层级 | 文件数 | 用例数 | 工具 |
|------|:------:|:------:|------|
| Jest 单元测试 (WMS) | 32 | ~1752 | Jest + jsdom + Istanbul 手动插桩 |
| Jest 单元测试 (WF/lib) | 16 | ~487 | Jest |
| Jest 单元测试 (scripts) | 1 | 25 | Jest (add-ids.js 纯函数) |
| BATS Shell 测试 | 15 | ~128 | BATS |
| pytest Python 测试 | 8 | ~107 | pytest |
| SQL 行为测试 | 8 | 95 | psql RAISE NOTICE 断言 |
...
| 指标 | 当前值 | 目标值 | 差距 |
|------|:------:|:------:|:----:|
| Jest Stmts | 94.96% | 95%+ | -0.04% |
| Jest Branch | 89.47% | 90%+ | -0.53% |
| 源文件有测试 | 93% | 95%+ | 2 个文件 |
