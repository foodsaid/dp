...
> **版本**: v0.8.1 | **日期**: 2026-03-21 | **适用**: 全新服务器部署
...
SSO:     dp-sso (Authelia 4.39.16, file backend + PG + Redis)
工作流:  dp-wf (n8n main, 路由+入队) + dp-wf-worker (n8n worker, 执行工作流)
监控:    dp-prometheus + dp-grafana + dp-alertmanager + 5 exporter
日志:    dp-loki (Loki 3.6, 日志聚合) + dp-alloy (Alloy 1.14, 日志收集)
```

8 个核心容器 (含 dp-wf-worker) + 2 SSO 容器 (profile: sso) + 10 监控容器 (profile: monitoring, 含 Loki + Alloy) + 3 可选容器 (tunnel/certbot/dns)
...
| 06 | wf06-items-sync.json | 物料缓存同步 | Cron 20:00 | ✅ |
| 07 | wf07-locations-sync.json | 仓库缓存同步 | Cron 20:15 | ✅ |
| 08 | wf08-stock-query.json | 库存查询 | Webhook | ✅ |
| 09 | wf09-stock-snapshot.json | 库存快照同步 | Cron 22:00 | ✅ |
| 10 | wf10-bin-sync.json | 库位缓存同步 | Cron 21:15 | ✅ |
...
| 20 | wf20-oms-sync.json | OMS SAP 订单同步 (Cron 21:30 + 手动) | Cron + 手动 | ✅ |
| 21 | wf21-oms-query.json | OMS 订单查询 + 明细 | Webhook | ✅ |
| 22 | wf22-oms-dd.json | OMS DD 拆单管理 | Webhook | ✅ |

### 5.11 Queue 模式 + Worker (v0.8.0+)

v0.8.0 起 n8n 从单进程 `regular` 模式升级为分布式 `queue` 模式:

- **dp-wf (main)**: 仅负责 webhook 路由 + 任务入队到 Redis Bull 队列
- **dp-wf-worker**: 消费队列并行执行工作流 (默认并发 10)
- **无自动降级**: 若 worker 宕机，任务在 Redis 堆积，main 不会接管执行；重启 worker 后自动消费

#### 相关环境变量

| 变量 | 默认值 | 说明 |
|------|--------|------|
| `N8N_WORKER_CONCURRENCY` | 10 | Worker 并发数 (通过 CLI flag 传入) |
| `N8N_CONCURRENCY_PRODUCTION_LIMIT` | 5 | Main 入队并发限制 (防 webhook 洪峰) |
| `EXECUTIONS_DATA_MAX_AGE` | 168 | 执行数据保留小时数 (7 天) |
...
```bash
# 1. 确认两个容器都 healthy
docker ps --filter "name=dp-wf" --format "table {{.Names}}\t{{.Status}}"

# 2. 触发一个 webhook，观察 worker 日志
docker logs dp-wf --tail 5        # 应显示 "Enqueued execution ..."
docker logs dp-wf-worker --tail 5  # 应显示 "Worker started/finished execution ..."

# 3. 检查 queue 指标 (仅 queue 模式下存在)
curl -s http://localhost:<DP_WF_PORT>/metrics | grep scaling_mode_queue
```
...
```bash
# 1. 在 .env 或 docker-compose.yml 中改回 EXECUTIONS_MODE=regular
# 2. 停止 worker + 重启 main
docker compose stop dp-wf-worker
docker compose up -d dp-wf
```
...
> SSO 使用 Authelia v4.39.16，通过 nginx `auth_request` 实现前置认证。默认关闭 (`DP_SSO_ENABLED=false`)，按需启用。
...
> Prometheus + Grafana + Alertmanager + Loki + Alloy 全栈监控, 通过 `profile: monitoring` 按需启动。
...
| 容器 | 镜像 | 职责 |
|------|------|------|
| dp-prometheus | prom/prometheus | 指标采集 + 告警评估 |
| dp-alertmanager | prom/alertmanager | 告警路由 + 通知 |
| dp-grafana | grafana/grafana-oss | 仪表板可视化 |
| dp-loki | grafana/loki 3.6 | 日志聚合存储 |
| dp-alloy | grafana/alloy 1.14 | 日志收集代理 (采集 Docker 容器日志 → Loki) |
| dp-node-exporter | prom/node-exporter | 宿主机指标 |
| dp-cadvisor | gcr.io/cadvisor | 容器资源指标 |
| dp-pg-exporter | prometheuscommunity/postgres-exporter | PostgreSQL 指标 |
| dp-redis-exporter-wf | oliver006/redis_exporter | Redis WF 队列指标 |
| dp-redis-exporter-bi | oliver006/redis_exporter | Redis BI 缓存指标 |
...
# Prometheus targets (应有 12 个 scrape target, 含 n8n main + worker)
curl -s http://localhost:8080/prometheus/api/v1/targets | python3 -m json.tool

# Loki 健康检查
curl -s http://localhost:8080/loki/ready
# → ready
...
13 组告警规则已预配置 (`infrastructure/monitoring/prometheus/rules/dp-alerts.yml`):
- 宿主机: CPU/内存/磁盘/磁盘预测
- 容器: 高内存/重启
- 服务: n8n/PG/Redis 不可用
- n8n 性能: 工作流失败率 / P95 延迟
- n8n 队列 (v0.4+): 队列积压 (N8nQueueBacklog) / Worker 离线 (N8nWorkerDown) / 失败激增 (N8nQueueFailedJobs)
...
| dp-loki | 3100 | — | — | 无 (仅网关) | monitoring |
| dp-alloy | 12345 | — | — | 无 | monitoring |
...
> 本手册基于 DP v0.8.1 编写。如有问题，请联系系统管理员。
