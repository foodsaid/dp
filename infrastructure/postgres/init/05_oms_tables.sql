-- ============================================================================
-- DP v0.8.0 — OMS Schema 完整建表 (PostgreSQL 17)
-- 订单管理系统: SAP 订单缓存 + DD 拆单 + 事件溯源预埋 + 审计日志
-- 含: P1 核心 4 表 + 2 视图 + 5 触发器 + Feature Flags + 触发器审计
-- ============================================================================
...
-- DEPRECATED v0.8: 已统一到 core.fn_*()，保留供向后兼容
-- ============================================================================
...
CREATE TABLE oms.orders (
    ...
    -- 业务状态 — DEPRECATED v0.8: 派生状态，由 execution_state + sap_status 推算，计划 v1.0 改为视图/计算列
...
-- v0.8: execution_state 复合索引 (替代 oms_status 单列索引)
CREATE INDEX idx_oms_orders_cc_exec_state ON oms.orders (company_code, doc_type, execution_state);
...
CREATE TABLE oms.order_lines (
    ...
    -- SAP 行类型 (WOR1.ItemType: 4=物料, 290=人工/间接费用)
    item_type INTEGER,
...
-- ============================================================================
-- 5. oms.trigger_audit_log — 跨 Schema 触发器观测日志 (v0.8+)
-- 量化触发频率，为 v1.0 事件驱动迁移提供数据支撑
-- ============================================================================
...
COMMENT ON TABLE oms.trigger_audit_log IS '跨 Schema 触发器观测日志 — 量化依赖强度，v1.0 迁移参考';
...
BEGIN
    ...
    -- 观测日志 (量化跨 Schema 触发频率，v1.0 事件驱动迁移参考)
