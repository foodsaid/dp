-- ============================================================================
-- DP v0.8 — WMS Schema 完整建表 (PostgreSQL 17)
...
-- DEPRECATED v0.8: 已统一到 core.fn_updated_at()，保留供向后兼容
-- ============================================================================
...
-- DEPRECATED v0.8: 已统一到 core.fn_enforce_company_code()，保留供向后兼容
-- ============================================================================
...
CREATE TABLE wms.wms_documents (
    ...
    wms_status VARCHAR(20) NOT NULL DEFAULT 'pending'  -- DEPRECATED v0.8: 统一使用 status，计划 v1.0 删除
...
-- 索引 (v0.8 优化: 复合索引替代单列索引)
CREATE INDEX idx_documents_cc_type_status ON wms.wms_documents (company_code, doc_type, status);
CREATE INDEX idx_documents_cc_sap_docnum ON wms.wms_documents (company_code, sap_doc_num) WHERE sap_doc_num IS NOT NULL;
...
-- DEPRECATED v0.8: 保留供 BI/Debug 兼容，建议使用复合索引
...
-- DEPRECATED v0.8: 已统一到 core.fn_synced_at()，保留供向后兼容
...
CREATE TABLE wms.wms_locations_cache (
    ...
    synced_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (company_code, whs_code)
);
...
CREATE TABLE wms.wms_bins_cache (
    ...
    synced_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (company_code, bin_code)
);
