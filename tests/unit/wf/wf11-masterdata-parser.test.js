...
describe('wf11-masterdata-parser.js - 主数据解析', () => {
    ...
    test('whs _json 为 null → 默认空数组 (触发 || 分支)', () => {
        const rows = [{ _type: 'whs', _json: null }];
        const result = parseMasterdata(rows);
        expect(result.warehouses.length).toBe(0);
    });

    test('bins _json 为 null → 默认空数组 (触发 || 分支)', () => {
        const rows = [{ _type: 'bins', _json: null }];
        const result = parseMasterdata(rows);
        expect(result.bins_map).toEqual({});
        expect(result.counts.bins).toBe(0);
    });

    test('whs _json 为 undefined → 默认空数组', () => {
        const rows = [{ _type: 'whs' }];
        const result = parseMasterdata(rows);
        expect(result.warehouses.length).toBe(0);
    });

    test('bins _json 为 undefined → 默认空数组', () => {
        const rows = [{ _type: 'bins' }];
        const result = parseMasterdata(rows);
        expect(result.counts.bins).toBe(0);
    });
});
