...
describe('wf21-query-builder.js - OMS 查询构建器', () => {
    ...
    describe('buildOmsQuery 基础查询', () => {
        ...
        test('场景 5b: date_from + date_to 参数索引不重叠 ($2/$3 各自独立)', () => {
            const result = buildOmsQuery({
                date_from: '2026-01-01',
                date_to: '2026-03-07'
            }, 'ACME');
            // company_code=$1, date_from=$2, date_to=$3
            expect(result.whereClause).toContain('$2');
            expect(result.whereClause).toContain('$3');
            expect(result.params[1]).toBe('2026-01-01');
            expect(result.params[2]).toBe('2026-03-07');
        });

        test('场景 5c: 仅 date_to (无 date_from) → 参数索引正确', () => {
            const result = buildOmsQuery({ date_to: '2026-03-07' }, 'ACME');
            // company_code=$1, date_to=$2
            expect(result.conditions.length).toBe(2);
            expect(result.whereClause).toContain('$2');
            expect(result.params[1]).toBe('2026-03-07');
        });
    });
