...
describe('buildItemsUpsertBatches — 物料 UPSERT', () => {
  ...
  test('超过 batchSize → 多批次', () => {
    ...
    var result = buildItemsUpsertBatches(items, 2);
...
describe('buildLocationsUpsert — 仓库 UPSERT', () => {
  test('正常仓库数据', () => {
    ...
    var result = buildLocationsUpsert(items);
    ...
    expect(result.sql).toContain('ON CONFLICT (whs_code) DO UPDATE');
    expect(result.count).toBe(2);
  });

  test('空数组 → SELECT 1', () => {
    var result = buildLocationsUpsert([]);
    expect(result.sql).toBe('SELECT 1');
    expect(result.count).toBe(0);
  });

  test('null → SELECT 1', () => {
    var result = buildLocationsUpsert(null);
    expect(result.sql).toBe('SELECT 1');
    expect(result.count).toBe(0);
  });
...
describe('buildBinsUpsertBatches — 库位 UPSERT', () => {
  test('正常库位数据', () => {
    ...
    var result = buildBinsUpsertBatches(items);
    expect(result).toHaveLength(1);
    ...
    expect(result[0].sql).toContain('ON CONFLICT (bin_code) DO UPDATE');
    expect(result[0].count).toBe(1);
  });

  test('空数组 → SELECT 1 占位', () => {
    var result = buildBinsUpsertBatches([]);
    expect(result).toHaveLength(1);
    expect(result[0].sql).toBe('SELECT 1');
    expect(result[0].count).toBe(0);
  });
  ...
  test('超过 batchSize → 多批次', () => {
    ...
    var result = buildBinsUpsertBatches(items, 3);
