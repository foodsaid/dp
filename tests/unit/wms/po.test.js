...
describe('po.js 分支覆盖补充', () => {
  ...
  test('buildPoOpenLines: lineStatus 缺失时默认为 O (SAP B1 open 行)', () => {
    // SAP B1 查询结果中 lineStatus 可能为 null/undefined，默认应视为 Open
    var lines = [{ lineNum: 0, itemCode: 'A', openQty: 10, quantity: 10, itemName: 'ItemA' }];
    var result = buildPoOpenLines(lines, {});
    expect(result).toHaveLength(1);
    expect(result[0].lineStatus).toBe('O');
  });

  test('buildPoOpenLines: lineStatus 为 null 时默认为 O', () => {
    var lines = [{ lineNum: 0, itemCode: 'A', openQty: 10, quantity: 10, lineStatus: null, itemName: '' }];
    var result = buildPoOpenLines(lines, {});
    expect(result).toHaveLength(1);
    expect(result[0].lineStatus).toBe('O');
  });

  test('buildPoOpenLines: SAP B1 LineNum=0 第一行正确参与过滤 (对账一致性)', () => {
    var lines = [
      { lineNum: 0, itemCode: 'A', openQty: 10, quantity: 10, lineStatus: 'O', itemName: '第一行' },
      { lineNum: 1, itemCode: 'B', openQty: 5, quantity: 5, lineStatus: 'O', itemName: '第二行' }
    ];
    var result = buildPoOpenLines(lines, {});
    expect(result).toHaveLength(2);
    expect(result[0].lineNum).toBe(0);
    expect(result[1].lineNum).toBe(1);
  });

  test('buildPoOpenLines: 可选字段缺失时使用默认值', () => {
    var lines = [{ lineNum: 0, itemCode: 'A', openQty: 10, quantity: 10 }];
    var result = buildPoOpenLines(lines, {});
    expect(result[0].itemName).toBe('');
    expect(result[0].whsCode).toBe('');
